...
/**
 * @description Normalize a date object to remove time components
...
/**
 * @description Test if given date is this week (Monday to Sunday)
 * @param {Date} d Date to test
...
exports.isThisWeek = function (d) {
  ...
  firstDay.setDate(today.getDate() - (dayOfWeek === 0 ? 6 : dayOfWeek - 1)); // Adjust for Monday as the first day

  // Calculate the last day of this week (Sunday)
  const lastDay = new Date(firstDay);
  lastDay.setDate(firstDay.getDate() + 6);

  const normalizedDate = normalizeDate(d);
  return normalizedDate >= firstDay && normalizedDate <= lastDay;
};

/**
 * @description Test if given date is next week (Monday to Sunday)
 * @param {Date} d Date to test
...
exports.isNextWeek = function (d) {
  ...
  // Calculate the first day of next week (Monday)
  const firstDay = new Date(today);
  firstDay.setDate(today.getDate() - (dayOfWeek === 0 ? 6 : dayOfWeek - 1) + 7); // Adjust for Monday as the first day

  // Calculate the last day of next week (Sunday)
  const lastDay = new Date(firstDay);
  lastDay.setDate(firstDay.getDate() + 6);
