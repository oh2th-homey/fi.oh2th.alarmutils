...
/**
 * Validates a cron time string.
 *
 * @param {string} cronTime - The cron time string to validate.
 * @returns {boolean} True if the cron time string is valid, false otherwise.
 */
exports.validateCronTime = function (cronTime) {
	// Regular expression to match the cron format without seconds
	// minutes * or 0-59
	// hours * or 0-23
	// days of month * or 1-31 or L
	// months * or 1-12
	// days of week * or 0-7 (0 or 7 is Sun)

	const minuteRegex = /^(\*|([0-5]?\d|\d{2})(-[0-5]?\d|\d{2})?)(\/[1-9]\d*)?(\-(\*|([0-5]?\d|\d{2})(-[0-5]?\d|\d{2})?)?)?(,(\*|([0-5]?\d|\d{2})(-[0-5]?\d|\d{2})?)(\/[1-9]\d*)?(\-(\*|([0-5]?\d|\d{2})(-[0-5]?\d|\d{2})?)?)?)*$/;
	const hourRegex = /^(\*|([01]?\d|2[0-3])(-[01]?\d|2[0-3])?)(\/[1-9]\d*)?(\-(\*|([01]?\d|2[0-3])(-[01]?\d|2[0-3])?)?)?(,(\*|([01]?\d|2[0-3])(-[01]?\d|2[0-3])?)(\/[1-9]\d*)?(\-(\*|([01]?\d|2[0-3])(-[01]?\d|2[0-3])?)?)?)*$/;
	const dayOfMonthRegex = /^(\*|([1-9]|[12]\d|3[01]|L)(-([1-9]|[12]\d|3[01]|L))?)(\/[1-9]\d*)?(\-(\*|([1-9]|[12]\d|3[01]|L)(-([1-9]|[12]\d|3[01]|L))?))?(,(\*|([1-9]|[12]\d|3[01]|L)(-([1-9]|[12]\d|3[01]|L)?))(\/[1-9]\d*)?(\-(\*|([1-9]|[12]\d|3[01]|L)(-([1-9]|[12]\d|3[01]|L))?))?)*/;
	const monthRegex = /^(\*|([1-9]|1[0-2])(-([1-9]|1[0-2]))?)(\/[1-9]\d*)?(\-(\*|([1-9]|1[0-2])(-([1-9]|1[0-2]))?)?)?(,(\*|([1-9]|1[0-2])(-([1-9]|1[0-2])?))(\/[1-9]\d*)?(\-(\*|([1-9]|1[0-2])(-([1-9]|1[0-2])?))?))*/;
	const dayOfWeekRegex = /^(\*|([0-7])|([0-7]-[0-7]))(\/[1-7])?(\-(\*|([0-7])|([0-7]-[0-7]))?)?(,(\*|([0-7])|([0-7]-[0-7]))(\/[1-7])?(\-(\*|([0-7])|([0-7]-[0-7]))?)?)*$/;

	// Combine the individual component regular expressions into the full cron regex
	const cronRegex = new RegExp(`${minuteRegex.source} ${hourRegex.source} ${dayOfMonthRegex.source} ${monthRegex.source} ${dayOfWeekRegex.source}`);

	// Test if the cronTime matches the regex pattern
	return cronRegex.test(cronTime);
};

/**
 * Check if the current time matches a cron expression with support for ranges.
 * @param {Date} datetime - The date to check.
 * @param {string} crontime - The cron expression to check.
 * @param {string} timezone - The timezone to use for current time calculations.
 * @returns {boolean} Returns true if the current time matches the cron expression, or false otherwise.
 */
exports.matchCronTime = function (datetime, crontime, timezone) {
	// Get the current date and time in the specified timezone
	const testDate = datetime.toLocaleString('en-US', { timeZone: timezone, hour12: false });
	const [date, time] = testDate.split(' ');
	const [hour, minute] = time.split(':');

	// Split the cron expression into its components
	const cronParts = crontime.split(' ');

	// Define arrays for each component
	const allowedMinutes = parseCronComponent(cronParts[0]);
	const allowedHours = parseCronComponent(cronParts[1]);
	const allowedDaysOfMonth = parseCronComponent(cronParts[2]);
	const allowedMonths = parseCronComponent(cronParts[3]);
	const allowedDaysOfWeek = parseCronComponent(cronParts[4]);

	// Check if the current time matches the cron expression
	if (
		allowedMinutes.includes(Number(minute)) &&
    allowedHours.includes(Number(hour)) &&
    (allowedDaysOfMonth.includes(Number(date)) || allowedDaysOfMonth.includes('*')) &&
    (allowedMonths.includes(new Date().getMonth() + 1) || allowedMonths.includes('*')) &&
    allowedDaysOfWeek.includes(new Date().getDay())
	) {
		return true;
	}
	return false;
...
/**
 * Parse a cron component string into an array of allowed values.
 * @param {string} component - The cron component string.
 * @returns {number[]} An array of allowed values.
 */
function parseCronComponent(component) {
	if (component === '*') {
		...
		const [step, divisor] = component.split('/').map(Number);
		return Array.from({ length: 60 }, (_, i) => i).filter((value) => value % divisor === 0);
	}
