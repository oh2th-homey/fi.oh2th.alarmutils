...
/**
 * @description Verify given time is valid cron expression
 * @param {String} t Time to test in HH:mm format
 * @returns {Boolean} True if time is valid cron expression
 */
exports.isTimeValidCron = function(t) {
  try {
    const { hh, mm } = t.split(':');
    const c = `* ${mm} ${hh} * * *`;
    cron.parseExpression(c);
    return true;
  } catch (error) {
    return false;
  }
