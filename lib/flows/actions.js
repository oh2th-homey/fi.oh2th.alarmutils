...
exports.init = async function(ctx) {
   ...
   * Device Action cards
   */
  const action_DEVICE_SCHEDULE_ENABLED = ctx.homey.flow.getActionCard('device_schedule_enabled');
  action_DEVICE_SCHEDULE_ENABLED.registerRunListener(async (args, state) => {
    await args.device.onAction_DEVICE_SCHEDULE_ENABLED(args.enabled);
  });
