...
exports.init = async function (ctx) {
   ...
   */
	...
	const action_DEVICE_SCHEDULE_CRONTIME = ctx.homey.flow.getActionCard('device_schedule_crontime');
	action_DEVICE_SCHEDULE_CRONTIME.registerRunListener(async (args, state) => {
		await args.device.onAction_DEVICE_SCHEDULE_CRONTIME(args.time);
	});
};
